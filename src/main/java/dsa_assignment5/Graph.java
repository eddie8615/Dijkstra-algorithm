...
public class Graph
{
		...
		throws GraphException
	{
		...
		List<Connection> connections = graph.remove(node);
		if(connections.size() < 2)
			throw new GraphException(String.format("Not enough number of elements for contracting"));
		if(connections.size() > 2)
			throw new GraphException(String.format("Too many elements"));
		Connection single = connections.remove(0);
		List<Connection> c1 = graph.get(single.getNode());
		Iterator<Connection> it1 = c1.iterator();
		Connection second = connections.remove(0);
		List<Connection> c2 = graph.get(second.getNode());
		Iterator<Connection> it2 = c2.iterator();
		while(it1.hasNext()) {
			Connection temp = it1.next();
		...
		while(it2.hasNext()) {
			Connection temp = it2.next();
			if(temp.getNode() == node) {
				c2.remove(temp);
				break;
			}
		}
		int newDis = single.getDistance() + second.getDistance();
		addEdge(single.getNode(), second.getNode(), newDis);
	}
		...
		throws GraphException
	{
		...
		Set<Integer> tight = new TreeSet<Integer>();
		int numOfNodes = graph.keySet().size();
		for(int i = 0; i < numOfNodes; i++) {
			minDistance.put(i, Integer.MAX_VALUE);
		}
		if(graph.get(node1) == null || graph.get(node2) == null)
			throw new GraphException(String.format("No such node in the graph"));
		tight.add(0);
		minDistance.put(0, 0);
		int lowestNode = 0;
		int currentNode = 0;
		for(int count = 0; count < numOfNodes; count++) {
			...
			List<Connection> conList = graph.get(lowestNode);
			Iterator<Connection> conIter = conList.iterator();
			Connection lowest = new Connection(0, 0);
			
			while(conIter.hasNext()) {
				Connection temp = conIter.next();
				if(temp.getDistance() <= min && !tight.contains(temp.getNode())) {
					min = temp.getDistance();
					lowest = temp;
				}
				if(lowest.getDistance() < lowestDistance) {
					if(tight.contains(temp.getNode()))
						continue;
					if(lowestNode != currentNode)
						tight.remove(lowestNode);
					lowestNode = lowest.getNode();
					lowestDistance = lowest.getDistance();
				}
				tight.add(temp.getNode());
				
				if(currentNode == 0)
					minDistance.put(temp.getNode(), temp.getDistance());
				
				List<Connection> nextNodeConList = graph.get(temp.getNode());
				Iterator<Connection> itr2 = nextNodeConList.iterator();
				
				while(itr2.hasNext()) {
					Connection temp1 = itr2.next();
					int newDis = minDistance.get(temp.getNode()) + temp1.getDistance();
					if(!tight.contains(temp1.getNode()) && 
						newDis < minDistance.get(temp1.getNode())){
							minDistance.put(temp1.getNode(), newDis);
					}
				}
			}
			currentNode = lowestNode;
		}
		return minDistance.get(node2);
	}
}
